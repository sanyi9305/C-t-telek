...
//TODO majd ráér később inicializálni, hogy ne lassítsa a szimulációt
  ...
  // beállítjuk az újonnan hozzáadott autó id értékét
  c->set_id(id);
...
//TODO majd ráér később inicializálni, hogy ne lassítsa a szimulációt
  ...
  // itt is beállítjuk az újonnan hozzáadott autó id értékét
  c->set_id(id);
                  ...
                  for ( auto c:m_smart_cars )
                    {
                      if ( c->get_type() == CarType::GANGSTER )
                        {
                          // a c->get_id segítségével leküldjük a kliensnek a
                          // megfelelő id-t
                          length += std::sprintf ( data+length,
