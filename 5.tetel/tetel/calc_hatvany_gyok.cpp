...
//c++-s könyv számológépe kibővítve hatványozással és gyökvonással
//! - négyzetre emelés
//_ - gyökvonás
...
/*
Gyokvonas hatvanyozas (csak egesz hatvany es gyok kitevokkel szamol ha 
lebegopontos szamot kap levagja a pont utan levo reszt)
!-negyzetre emeles 
_-gyokvonas
*/

double Parser::NGY(bool get)
{
...
for(;;)
	...
	case Lexer::GYOK:{
		...
		if(!(left<0)){				
			left=pow(left,1/double(d));		
			break;
			}
		else{
			if(d%2){
				left=-pow(abs(left),1/double(d));		
				break;		
				}	
		}
		
		throw Error::Negativ_gyok();
		break;}
	default:
	     return left;
}
...
int main(int argc, char* argv[])
{
	...
	Symbol_table::table["e"]  = 2.7182818284590452354;
 	...
 	while (*input) {
		...
		catch(Error::Negativ_gyok){
			cerr<< "nem ismerem a komplex szamokat\n";
			skip();
		}		
